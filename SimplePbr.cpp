...
Update(double Time, float DeltaTime)
{
    ...
    TFrameResources& Frame = GFrames[Dx::GFrameIndex];
    {
        ...
        *Ptr++ = XMFLOAT2(-0.5f + Misc::Randomf(-0.05f, 0.05f), -0.5f + Misc::Randomf(-0.05f, 0.05f));
        *Ptr++ = XMFLOAT2(0.5f + Misc::Randomf(-0.05f, 0.05f), 0.15f + Misc::Randomf(-0.05f, 0.05f));
    }
    ...
    Dx::GCmdList->OMSetRenderTargets(1, &Dx::GetBackBufferRtv(), 0, nullptr);
    ...
    Dx::GCmdList->SetGraphicsRootDescriptorTable(0, Dx::CopyDescriptorsToGpu(1, GCanvasSrv));
...
static void
InitializeFrameResources(unsigned FrameIndex)
{
...
InitializeResources()
{
    // Canvas resources
    {
        ...
        Dx::AllocateDescriptors(D3D12_DESCRIPTOR_HEAP_TYPE_RTV, 1, GCanvasRtv);
        Dx::GDevice->CreateRenderTargetView(GCanvasTex, nullptr, GCanvasRtv);

        Dx::AllocateDescriptors(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV, 1, GCanvasSrv);
    ...
    {
        eastl::vector<uint8_t> CsoVs = Misc::LoadFile("Data/Shaders/DisplayCanvas.vs.cso");
        eastl::vector<uint8_t> CsoPs = Misc::LoadFile("Data/Shaders/DisplayCanvas.ps.cso");
    ...
    // Line shaders
    {
        eastl::vector<uint8_t> CsoVs = Misc::LoadFile("Data/Shaders/Sand.vs.cso");
        eastl::vector<uint8_t> CsoPs = Misc::LoadFile("Data/Shaders/Sand.ps.cso");
...
Update(double Time, float DeltaTime)
{
    Priv::Update(Time, DeltaTime);
}

static void
Initialize()
{
    Priv::InitializeResources();
    Priv::InitializeFrameResources(0);
    Priv::InitializeFrameResources(1);
}
